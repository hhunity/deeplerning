...
各モデルが**回帰**か**分類**かを整理した表です。

| モデル | ディレクトリ | タスク種別 | 損失関数 | 出力 | 予測内容 |
|--------|------------|-----------|---------|------|---------|
| **EfficientNet-B0** | `efficientnet/` | **回帰** | HuberLoss | スカラー値 (1ニューロン) | 結晶数を連続値で予測（例: 2.5個） |
| **ResNet** | `resnet/` | **分類** | CrossEntropyLoss | クラス確率 (num_classes) | 結晶数をクラスとして分類（0個・1個・2個…） |
| **CSRNet** | `crnet/` | **回帰**（密度マップ） | MSELoss | 密度マップ（ピクセルごとの連続値） | 各ピクセルの密度値 → 積分してカウント |
| **U-Net** | `unet/` | **セグメンテーション** | DiceBCELoss | 二値マスク | 結晶領域を分割 → 連結成分ラベリングでカウント |
| **SAM2** | `sam/` | **インスタンスセグメンテーション** | なし（学習なし） | インスタンスマスク群 | ゼロショットで領域検出 → マスク数をカウント |
| **DQN** | `dqn/` | **強化学習** | MSELoss (Q学習) | Q値（3アクション） | FX売買判断（結晶カウントとは無関係） |

### ポイント
- **EfficientNet（回帰）**: 連続値を直接予測。データが少なくても学習しやすい。**メインのアプローチ**。
- **ResNet（分類）**: 個数をクラスに変換。クラスバランスに注意が必要。
- **CSRNet（密度マップ回帰）**: 混雑カウント手法を転用。アノテーションはポイント座標が必要。
- **U-Net（セグメンテーション）**: ピクセル単位で結晶領域を分割してからカウント。マスクアノテーションが必要。
- **DQN**: 結晶カウントプロジェクトとは無関係のFX強化学習。
...
## バッチ検証の指標（MAE / RMSE / R²）の読み方

`validate` コマンドで出力される3つの指標の解釈方法です。

```bash
python efficientnet_main.py validate --images ./data/images --annotations ./data/annotations.json
```

### 各指標の意味

**MAE（平均絶対誤差）**
「平均で何個ズレているか」をそのまま表す。最も直感的な指標。
- MAE = 1.5 → 平均1.5個ズレ
- MAE = 5.0 → 平均5個ズレ → 実用的にNG

**RMSE（二乗平均平方根誤差）**
大きくズレたサンプルにペナルティが大きい。
- MAEよりRMSEが大幅に大きい → 「たまに大爆死している画像がある」サイン
- scatter_plot.png の外れ点を確認すべき

**R²（決定係数）**
モデルが「結晶数の増減」をどれだけ説明できているか。
- 1.0 → 完璧
- 0.9以上 → 良好
- 0.7未満 → ほぼ平均値を予測しているだけ
- 0以下 → 平均を返す方がマシ
...
| 状況 | 次の手 |
|------|--------|
| MAE大 + R²低 | モデルが学習できていない → エポック数・LR・データ量を見直す |
| MAE小 + R²低 | 結晶数の分布が偏っている（0個ばかりなど）→ データを確認 |
| RMSE >> MAE | 特定画像で大失敗 → scatter_plot.png の外れ点を `infer --gradcam` で確認 |
| R² > 0.9 でも MAE大 | 正解ラベルの範囲が広い（0〜50個など）→ 許容誤差の基準を見直す |

### 目標の目安

画像の結晶数が大体5〜15個なら **MAE < 1.5** を目標にすると良い。
「結晶数の典型的な範囲に対してMAEが何%か」で判断するのが実用的。
